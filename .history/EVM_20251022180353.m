...
sigT  = [0.42 0.38 0.38 0.42];   % 调大 sigma，使穹顶更集中
amp   = [1.05 0.95 0.90 0.85];

% 抬高穹顶，并缩小每个穹顶的覆盖范围（为每个穹顶使用局部网格）
zlift      = 0.22;   % 统一抬高量（不改变颜色映射）
spanFactor = 2.1;    % 每个穹顶半宽 = spanFactor * sigma（越小覆盖越小）
gridN      = 121;    % 局部网格分辨率
...
% ---------- 5) 轴、视角与观感 ----------
xlim([-2 2]); ylim([-1 3]); zlim([0 1.7]); view(47,21);
...
exportgraphics(gcf,'images/EVM.png','Resolution',300);