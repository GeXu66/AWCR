...
from feature_library import build_feature_library, compute_feature_for_name, extract_condition_prefix, compute_feature_for_segment_arrays
...
def pipeline_exp3(trainlist: List[List[str]],
                  longlist: List[str],
                  channel_in: List[int],
                  ...
                  future_order: int = 50,
                  mean_flag: bool = True,
                  results_dir: str = os.path.join('result', 'EXP3')) -> None:
    os.makedirs(results_dir, exist_ok=True)

    # Merge training groups by coarse prefix
    coarse_to_names: Dict[str, List[str]] = {}
    for group in trainlist:
        prefix = extract_condition_prefix(group)
        coarse_to_names.setdefault(prefix, []).extend(group)
    merged_trainlist: List[List[str]] = list(coarse_to_names.values())

    # Train feature library and EVM recognizer
    build_feature_library(merged_trainlist, feature_dir='feature')
    ...
    # MIMOFIR manager and ensure models
    manager = MIMOFIRManager(past_order, future_order, channel_in, channel_out, mean_flag, models_dir='models')
    for group in merged_trainlist:
        prefix = extract_condition_prefix(group)
        if not manager.exists(prefix):
            manager.train_from_files(prefix, group, piece='all')

    # Load long concatenated U,Y for provided long list
    U_all, Y_all = read_io_for_names(longlist, channel_in, channel_out, mean_flag)
    n_samples = Y_all.shape[0]
    start_idx = int(0.10 * n_samples)
    end_idx = int(0.80 * n_samples)
    U_slice = U_all[start_idx:end_idx]
    Y_slice = Y_all[start_idx:end_idx]

    fs = 500
    seg_len = int(5 * fs)
    num_segs = max(0, (Y_slice.shape[0] // seg_len))

    # Mapping class names to ids
    known_classes = sorted(list(coarse_to_names.keys()))
    class_to_id = {c: i + 1 for i, c in enumerate(known_classes)}
    unknown_labels = ['Unknown1', 'Unknown2', 'Unknown3']
    for i, u in enumerate(unknown_labels):
        class_to_id[u] = len(known_classes) + 1 + i

    # Storage
    Y_pred_concat = []
    class_id_series = np.zeros(Y_slice.shape[0], dtype=np.int32)
    ...
    for k in range(num_segs):
        ...
        # Build 1D projection from inputs (StandardScaler + PCA(1))
        scaler = StandardScaler()
        u_scaled = scaler.fit_transform(u_seg)
        pca = PCA(n_components=1)
        x_1d = pca.fit_transform(u_scaled).reshape(-1)

        # STFT on 1D projection
        _, _, Z = stft(x_1d, fs=fs, nperseg=32)
        Zabs = np.abs(Z)

        # Segment feature compatible with EVM features
        feat = compute_feature_for_segment_arrays(Zabs, x_1d)

        # Predict class using EVM recognizer
        if hasattr(recognizer, 'predict_on_feature'):
            cls_hat, prob, scores = recognizer.predict_on_feature(feat)
        else:
            # Fallback: treat as unknown
            cls_hat, prob, scores = 'Unknown', 0.0, {}

        if cls_hat == 'Unknown':
            # fallback to most probable known class for estimation
            est_cls = None
            if len(scores) > 0:
                est_cls = max(scores, key=scores.get)
            else:
                # if scores empty, default to first known
                est_cls = known_classes[0]
            class_id = class_to_id[unknown_labels[0]]  # map to Unknown1 by default
        else:
            est_cls = cls_hat
            class_id = class_to_id.get(cls_hat, class_to_id[unknown_labels[0]])

        # Estimation with chosen class model
        Theta = manager.load(est_cls)
        if Theta is None:
            # train from merged group corresponding to est_cls
            group_for_cls: List[str] = []
            for grp in merged_trainlist:
                if extract_condition_prefix(grp) == est_cls:
                    group_for_cls = grp
                    break
            if group_for_cls:
                Theta = manager.train_from_files(est_cls, group_for_cls, piece='all')
            else:
                # cannot estimate; fill zeros
                Y_pred_seg = np.zeros_like(y_seg)
                Y_pred_concat.append(Y_pred_seg)
                class_id_series[s:e] = class_id
                continue

        Y_pred_seg = manager.estimate(Theta, u_seg)
        Y_pred_concat.append(Y_pred_seg)
        class_id_series[s:e] = class_id

    if len(Y_pred_concat) == 0:
        print('Warning: no segments generated for EXP3 experiment.')
        return

    Y_pred = np.vstack(Y_pred_concat)

    # Save combined outputs for plotting
    class_names = known_classes + unknown_labels
    out_path = os.path.join(results_dir, 'combined.mat')
    scio.savemat(out_path, {
        'Y_true': Y_slice,
        'Y_pred': Y_pred,
        'class_ids': class_id_series,
        'seg_len': seg_len,
        'fs': fs,
...
if __name__ == '__main__':
    ...
    if EXP == 1:
        ...
        testlist = [
            ['SW20_03', 'SW25_03', 'SW35_03', 'RE40_01', 'RE40_02', 'CJ50_01', 'CJ50_02']
        ]
    else:
        trainlist = [
    ...
    if EXP == 1:
        ...
        results_dir = os.path.join('result', 'EXP3')
        # longlist is the first (and only) element of testlist
        longlist = testlist[0]
        pipeline_exp3(trainlist, longlist, channel_in, channel_out, past_order, future_order, mean_flag, results_dir)
    else:
