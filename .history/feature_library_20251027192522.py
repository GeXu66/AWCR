...
def _compute_wst_features_per_channel(x: np.ndarray, fs: int, bands: List[Tuple[float, float]], max_coeffs: int) -> np.ndarray:
    """
    ...
    try:
        ...
        if feat.ndim > 1:
            feat = feat.reshape(-1)
        if feat.shape[0] > max_coeffs:
            feat = feat[:max_coeffs]
        ...
        try:
            ...
            feat = np.concatenate([mean_amp, std_amp])
            if feat.shape[0] > max_coeffs:
                feat = feat[:max_coeffs]
            return feat.astype(np.float64)
        except Exception:
            ...
            feat = np.asarray(feats, dtype=np.float64)
            if feat.shape[0] > max_coeffs:
                feat = feat[:max_coeffs]
            return feat
...
def _compute_wst_msc_feature(name: str) -> np.ndarray:
    ...
    per_dim = int(getattr(config, 'WST_MAX_COEFFS', 64))
    # Per-channel WST-like features
    feats_wst = []
    for c in range(X.shape[1]):
        feats_wst.append(_compute_wst_features_per_channel(X[:, c], fs, bands, per_dim))
    # Pad/truncate each channel vector to fixed per_dim
    padded = []
    for fch in feats_wst:
        if fch.shape[0] < per_dim:
            pad = np.zeros(per_dim - fch.shape[0], dtype=np.float64)
            padded.append(np.concatenate([fch, pad]))
        else:
            padded.append(fch[:per_dim])
