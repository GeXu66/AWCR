...
def stability_selection_scores(X: np.ndarray,
                               Y: np.ndarray,
                               n_bootstrap: int = 100,
    ...
    """
    Compute feature stability scores using MultiTaskLasso over bootstrap subsamples.
    Returns scores in [0,1] per feature indicating selection frequency.
    """
    rng = np.random.RandomState(random_state)
    n_samples, n_features = X.shape
    if alphas is None:
        # a mild range; user can tune
        alphas = np.logspace(-4, 0, 6)
    counts = np.zeros(n_features, dtype=float)

    for _ in range(n_bootstrap):
        idx = rng.choice(n_samples, size=int(n_samples * subsample_frac), replace=False)
        Xb, Yb = X[idx, :], Y[idx, :]
        # standardize per subsample for lasso stability
        Xb = Xb - np.mean(Xb, axis=0, keepdims=True)
        Yb = Yb - np.mean(Yb, axis=0, keepdims=True)
        selected = np.zeros(n_features, dtype=bool)
        for alpha in alphas:
            mtl = MultiTaskLasso(alpha=alpha, fit_intercept=False, max_iter=5000)
            mtl.fit(Xb, Yb)
            sel_alpha = np.any(np.abs(mtl.coef_) > 1e-12, axis=0)  # (n_features,)
...
def run_multitask_lasso_selection(past_order: int,
    ...
    # build design matrix with noncausal lags
    Phi = build_noncausal_design(U, past_order, future_order)

    # compute stability scores at FEATURE-GROUP level (per input channel across all time-shifts)
    # approach: aggregate selection across all shifted features, then average per channel
    scores_feature = stability_selection_scores(Phi, Y, n_bootstrap=n_bootstrap,
                                                subsample_frac=subsample_frac, alphas=alphas,
