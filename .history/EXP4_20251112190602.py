...
def _parse_base_names(arg: str) -> List[str]:
    if not arg:
        return []
...
def simulate_full_load_inputs(U: np.ndarray,
                              scale_gain: float = 0.4,
                              bias_scale: float = 0.05) -> np.ndarray:
    """
    Increase overall load by scaling inputs and adding a bias proportional to the
    channel-wise standard deviation.
    """
    bias1 = bias_scale * np.std(U[:,4], axis=0, keepdims=True)
    bias2 = bias_scale * np.std(U[:,5:], axis=0, keepdims=True)
    U[:,4] = U[:,4] * (1.0 - scale_gain) + bias1
    U[:,5:] = U[:,5:] * (1.0 + scale_gain) + bias2
    return U
...
def simulate_sport_suspension_inputs(U: np.ndarray,
                                     stiffness_gain: float = 0.6) -> np.ndarray:
    """
    ...
    bias1 = bias_scale * np.std(U[:,4], axis=0, keepdims=True)
    bias2 = bias_scale * np.std(U[:,5:], axis=0, keepdims=True)
    U[:,4] = U[:,4] * (1.0 - stiffness_gain) + bias1
    U[:,5:] = U[:,5:] * (1.0 + stiffness_gain) + bias2
    return U
...
def _build_scenarios(args: argparse.Namespace) -> List[Dict[str, Union[str, List[List[str]]]]]:
    base_names = _parse_base_names(args.base_name)
    scenarios: List[Dict[str, Union[str, List[List[str]]]]] = []
    user_label = args.scenario_name.strip()
    if base_names:
        multi = len(base_names) > 1
        for base in base_names:
            scenario_name = user_label if (user_label and not multi) else (
                f'{user_label}_{base}' if user_label else base
            )
            scenarios.append({
                'trainlist': [[f'{base}_01']],
                'testlist': [[f'{base}_02']],
                'changelist': [[f'{base}_03']],
                'scenario_name': scenario_name
            })
    else:
        trainlist = _normalize_groups(_parse_group_string(args.trainlist))
        testlist = _normalize_groups(_parse_group_string(args.testlist))
        changelist = _normalize_groups(_parse_group_string(args.changelist))
        ...
        scenario_name = user_label if user_label else _condition_token(trainlist[0][0])
        scenarios.append({
            'trainlist': trainlist,
            'testlist': testlist,
...
def main() -> None:
    ...
    for scenario in scenarios:
        print(f'Running EXP4 for scenario "{scenario["scenario_name"]}"...')
        theta_cache = prepare_models(scenario['trainlist'], manager)
        output_dir = os.path.join(args.output_dir, scenario['scenario_name'])
        _ensure_dir(output_dir)

        run_loading_conditions(manager, theta_cache, scenario['testlist'], scenario['changelist'],
                               args.sample_rate, args.fz_index, output_dir)
        run_tire_changes(manager, theta_cache, scenario['testlist'], scenario['changelist'],
                         args.sample_rate, args.fz_index, output_dir)
        run_suspension_mods(manager, theta_cache, scenario['testlist'], scenario['changelist'],
                            args.sample_rate, args.fz_index, output_dir)
    print('EXP4 experiments completed for all scenarios.')
