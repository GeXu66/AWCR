...
% Fz-only MAPE for panels (a), (b), (d)
fz_mape1 = 100.0 * mean(abs((Yp1(:, fz_idx) - Yt1(:, fz_idx)) ./ max(abs(Yt1(:, fz_idx)), eps)));
fz_mape2 = 100.0 * mean(abs((Yp2(:, fz_idx) - Yt2(:, fz_idx)) ./ max(abs(Yt2(:, fz_idx)), eps)));
fz_mape3 = 100.0 * mean(abs((Yp3(:, fz_idx) - Yt3(:, fz_idx)) ./ max(abs(Yt3(:, fz_idx)), eps)));
...
xlim([0, max(t1)]);
text(0.02, 0.90, sprintf('MAPE=%.1f%%', fz_mape1), 'Units', 'normalized', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', legend_fontsize);
xlim([0, max(t1)]);
grid on;
