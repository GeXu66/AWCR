...
legend({'ROC', 'Random', 'Op. point'}, 'Location', 'southeast', 'FontSize', legend_size);
...
ax = gca; ax.YTick = 1:numel(feat_sorted); ax.YTickLabel = feat_sorted; ax.TickLabelInterpreter = 'latex'; ax.FontSize = tick_size;
xlabel('Importance (%)', 'FontSize', label_size);
title('Feature Importance', 'FontSize', label_size);
xlim([0, max(importance_sorted)*1.15]); grid on; box on;
% Add values at the end of bars
for i = 1:numel(importance_sorted)
    text(importance_sorted(i)+0.5, i, sprintf('%.0f%%', importance_sorted(i)), 'VerticalAlignment', 'middle', 'FontSize', tick_size);
end
...
%% 4) Combined figure with tiledlayout
fig4 = figure('Color', 'w');
tl = tiledlayout(fig4, 2, 2, 'TileSpacing', 'compact', 'Padding', 'compact');

% Top row: confusion matrix spanning two columns
nexttile([1 2]);
imagesc(C);
axis tight;
colormap(parula(256));
cb4 = colorbar; cb4.Label.String = 'Count'; cb4.Label.FontSize = cb_label_size;
% Place colorbar at layout edge if supported (avoids stealing column width)
if isprop(cb4, 'Layout') && isprop(cb4.Layout, 'Tile')
    cb4.Layout.Tile = 'east';
end
caxis([0 9]);
% hide the tick of the axis but show the XTickLabel and YTickLabel
set(gca, 'XTick', []);
set(gca, 'YTick', []);
set(gca, 'XTickLabel', outputs, 'XTickLabelRotation', 0, 'FontSize', tick_size);
set(gca, 'YTickLabel', inputs, 'FontSize', tick_size);
...
for r = 1:nr4
    for c = 1:nc4
        txtColor = 'k';
        if C(r,c) > 6, txtColor = 'k'; end
        text(c, r, sprintf('%d', C(r,c)), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'Color', txtColor, 'FontSize', 10, 'FontWeight', 'bold');
    end
end
box on;
axA = gca;
text(axA, -0.07, 1.05, '(a)', 'Units', 'normalized', 'FontSize', label_size, 'FontWeight', 'bold', 'Clipping', 'off');
% Bottom-left: ROC
nexttile;
plot(FPR, TPR, 'b-', 'LineWidth', 2); hold on;
plot([0 1], [0 1], 'k--', 'LineWidth', 1);
xlabel('False Positive Rate', 'FontSize', label_size); ylabel('True Positive Rate', 'FontSize', label_size);
title(sprintf('ROC (AUC = %.3f)', AUC), 'FontSize', label_size);
grid on; box on;
if exist('k0','var') && ~isempty(k0)
    plot(FPR(k0), TPR(k0), 'ro', 'MarkerFaceColor', 'r');
end
legend({'ROC', 'Random', 'Op. point'}, 'Location', 'southeast', 'FontSize', legend_size-2);

% Subplot label (b) outside top-left
axB = gca;
text(axB, -0.17, 1.07, '(b)', 'Units', 'normalized', 'FontSize', label_size, 'FontWeight', 'bold', 'Clipping', 'off');

% Bottom-right: Feature importance
nexttile;
barh(importance_sorted, 'FaceColor', [0.30 0.65 0.93]);
set(gca, 'YDir', 'reverse');
ax4 = gca; ax4.YTick = 1:numel(feat_sorted); ax4.YTickLabel = feat_sorted; ax4.TickLabelInterpreter = 'latex'; ax4.FontSize = tick_size - 3;
xlabel('Importance (%)', 'FontSize', label_size);
xlim([0 23]);
title('Feature Importance', 'FontSize', label_size);
grid on; box on;
for i = 1:numel(importance_sorted)
    text(importance_sorted(i)+0.5, i, sprintf('%.0f%%', importance_sorted(i)), 'VerticalAlignment', 'middle', 'FontSize', tick_size - 2);
end

% Subplot label (c) outside top-left
axC = gca;
text(axC, -0.15, 1.07, '(c)', 'Units', 'normalized', 'FontSize', label_size, 'FontWeight', 'bold', 'Clipping', 'off');

% Export combined
exportgraphics(fig4, fullfile(outDir, 'exp2_combined.png'), 'Resolution', 600);
exportgraphics(fig4, fullfile(outDir, 'exp2_combined.pdf'), 'Resolution', 600);
