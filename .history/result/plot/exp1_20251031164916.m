...
% Fz-only MAPE for panels (a), (b), (d) with mean restored (+4000)
mape_bias = 4000;
fz_true1 = Yt1(:, fz_idx) + mape_bias; fz_pred1 = Yp1(:, fz_idx) + mape_bias;
fz_true2 = Yt2(:, fz_idx) + mape_bias; fz_pred2 = Yp2(:, fz_idx) + mape_bias;
fz_true3 = Yt3(:, fz_idx) + mape_bias; fz_pred3 = Yp3(:, fz_idx) + mape_bias;
fz_mape1 = 100.0 * mean(abs((fz_pred1 - fz_true1) ./ max(abs(fz_true1), eps)));
fz_mape2 = 100.0 * mean(abs((fz_pred2 - fz_true2) ./ max(abs(fz_true2), eps)));
fz_mape3 = 100.0 * mean(abs((fz_pred3 - fz_true3) ./ max(abs(fz_true3), eps)));
...
nexttile;
plot(t1, Yt1(:, fz_idx), 'Color', 'r', 'LineWidth', 1.5); hold on;
...
text(0.02, 0.15, '(a)', 'Units', 'normalized', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', label_fontsize);
xlim([0, max(t1)]);
grid on;
...
nexttile;
plot(t2, Yt2(:, fz_idx), 'Color', 'r', 'LineWidth', 1.5); hold on;
...
text(0.02, 0.15, '(b)', 'Units', 'normalized', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', label_fontsize);
xlim([0, max(t1)]);
grid on;
% title('(b) Estimation performance for F_z during the second encounter with BC50');

% (c) Error comparison first vs second (smoothed with shaded area)
nexttile;
hold on;
smoothN = max(1, round(0.05 * fs)); % 50 ms window
e1s = movmean(e1, smoothN);
e2s = movmean(e2, smoothN);
c1 = [0.85 0.33 0.10];
c2 = [0.49 0.18 0.56];
% shaded area from 0 to error
fill([t1, fliplr(t1)], [zeros(1, numel(e1s)), fliplr(e1s')], c1, 'FaceAlpha', 0.15, 'EdgeColor', 'none', 'HandleVisibility', 'off');
fill([t2, fliplr(t2)], [zeros(1, numel(e2s)), fliplr(e2s')], c2, 'FaceAlpha', 0.15, 'EdgeColor', 'none', 'HandleVisibility', 'off');
% smoothed error lines on top
plot(t1, e1s, '-', 'Color', c1, 'LineWidth', 1.4);
plot(t2, e2s, '-', 'Color', c2, 'LineWidth', 1.4);
xlabel('Time (s)', 'FontSize', label_fontsize); ylabel('|Error|', 'FontSize', label_fontsize);
legend({'BC50\_01', 'BC50\_02'}, 'Location', 'northeast', 'FontSize', legend_fontsize);
maxT = max([t1(:)]);
xlim([0, maxT]);
grid on; box on;
...
text(0.02, 0.15, '(c)', 'Units', 'normalized', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', label_fontsize);
% title('(c) Comparison of initial error (1st) vs adapted error (2nd)');

% (d) Novel condition RC30 (first encounter)
nexttile;
plot(t3, Yt3(:, fz_idx), 'Color', 'r', 'LineWidth', 1.5); hold on;
...
text(0.02, 0.15, '(d)', 'Units', 'normalized', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', label_fontsize);
grid on;
...
rootDir = fileparts(resultDir); % project root (parent of 'result')
outDir = fullfile(rootDir, 'images', 'exp1');
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
exportgraphics(fig, fullfile(outDir, 'exp1.png'), 'Resolution', 600);
exportgraphics(fig, fullfile(outDir, 'exp1.pdf'), 'Resolution', 600);
end
