...
% Read MAPE from metrics.txt (library-computed); if missing, fallback to formula
...
mape3 = read_mape(expDir, s3, dir3);
if isnan(mape3)
    mape3 = 100.0 * mean(abs((Yp3(:, fz_idx) - Yt3(:, fz_idx)) ./ max(abs(Yt3(:, fz_idx)), eps)));
end
...
xlim([0, max(t1)]);
text(0.02, 0.90, sprintf('MAPE=%.1f%%', mape1), 'Units', 'normalized', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', legend_fontsize);
xlim([0, max(t1)]);
grid on;
...
xlim([0, max(t1)]);
text(0.02, 0.90, sprintf('MAPE=%.1f%%', mape2), 'Units', 'normalized', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', legend_fontsize);
xlim([0, max(t1)]);
grid on;
...
xlim([0, max(t3)]);
text(0.02, 0.90, sprintf('MAPE=%.1f%%', mape3), 'Units', 'normalized', 'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', legend_fontsize);
xlim([0, max(t3)]);
grid on;
