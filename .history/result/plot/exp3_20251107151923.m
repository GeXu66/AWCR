...
% subplot label (a)
ax1 = gca;
text(ax1, 0.06, 0.95, '(a)', 'Units','normalized', 'FontSize', label_fontsize, 'FontWeight','bold', 'Clipping','off');
...
text(0.01, 0.23, '1-25 Known Conditions', 'Units', 'normalized', 'FontSize', axis_fontsize);
text(0.01, 0.1, '26-28 Unknown Conditions', 'Units', 'normalized', 'FontSize', axis_fontsize);
% subplot label (b)
ax2 = gca;
text(ax2, -0.06, 1.05, '(b)', 'Units','normalized', 'FontSize', label_fontsize, 'FontWeight','bold', 'Clipping','off');
...
for i = 1:3
    ...
    [f, xi] = ksdensity(ei);
    plot(axL, xi, f, 'Color', colorsF(i,:), 'LineWidth', 1.8, 'DisplayName', labels{i});
    % fill under curve with light alpha
    fill(axL, [xi, fliplr(xi)], [f, zeros(size(f))], colorsF(i,:), 'FaceAlpha', 0.15, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    mu = mean(ei);
    xline(axL, mu, '--', 'Color', colorsF(i,:), 'LineWidth', 1.0, 'HandleVisibility', 'off');
end
...
xlim([-5e3 5e3]);
legend(axL,'Location', 'northeast', 'FontSize', legend_fontsize, 'Interpreter', 'latex');
% subplot label (c)
text(axL, -0.10, 1.05, '(c)', 'Units','normalized', 'FontSize', label_fontsize, 'FontWeight','bold', 'Clipping','off');
...
for j = 1:3
    ...
    [f, xi] = ksdensity(ej);
    plot(axR, xi, f, 'Color', colorsM(j,:), 'LineWidth', 1.8, 'DisplayName', labels{j});
    fill(axR, [xi, fliplr(xi)], [f, zeros(size(f))], colorsM(j,:), 'FaceAlpha', 0.15, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    mu = mean(ej);
    xline(axR, mu, '--', 'Color', colorsM(j,:), 'LineWidth', 1.0, 'HandleVisibility', 'off');
end
...
xlim([-2e3 2e3]);
legend(axR,'Location', 'northeast', 'FontSize', legend_fontsize, 'Interpreter', 'latex');
% subplot label (d)
text(axR, -0.10, 1.05, '(d)', 'Units','normalized', 'FontSize', label_fontsize, 'FontWeight','bold', 'Clipping','off');
