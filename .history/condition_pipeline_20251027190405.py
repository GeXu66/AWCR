...
def pipeline(trainlist: List[List[str]],
    ...
    recognizer = get_recognizer(
        ...
        # DTW parameters
        dtw_window_ratio=getattr(config, 'DTW_WINDOW_RATIO', 0.1),
        dtw_downsample=getattr(config, 'DTW_DOWNSAMPLE', 4),
        dtw_reject_quantile=getattr(config, 'DTW_REJECT_QUANTILE', 0.9),
        dtw_margin=getattr(config, 'DTW_MARGIN', 1.2),
        channel_in=channel_in, mean_flag=mean_flag,
    )
