...
scatter3(X_dot(:,1),   X_dot(:,2),   z0+zeros(size(X_dot,1),1),   18, [0.35 0.60 1.00], 'filled');  % 蓝色小点
scatter3(X_diam(:,1),  X_diam(:,2),  z0+zeros(size(X_diam,1),1),  60, 'g', 'd', 'filled', 'MarkerEdgeColor',[0 0.45 0]); % 绿菱形
scatter3(X_square(:,1),X_square(:,2),z0+zeros(size(X_square,1),1),60, [0 0.25 0.85], 's', 'filled'); % 蓝方块
scatter3(X_star(:,1),  X_star(:,2),  z0+zeros(size(X_star,1),1),  70, [1 0 1], '*', 'LineWidth',1.0); % 洋红星
...
for k = 1:size(EV,1)
    ...
    [~, h] = contour3(xg,yg,Z,levels);   % 显式拿到句柄在第二个输出
    set(h,'LineWidth',1.25);             % 创建后再设线宽

    % —— 将等高线压到 z=0（多重回退以兼容不同版本）——
    if isprop(h,'ZLocation')
        try
            set(h,'ZLocation',0);        % 优先用数值 0（多数版本都支持）
        catch
            try
                set(h,'ZLocation','zmin');   % 某些版本支持关键字
            catch
                % 回退：逐个把子线条的 ZData 设为 0
                ch = get(h,'Children');
                for ic = 1:numel(ch)
                    zd = get(ch(ic),'ZData');
                    set(ch(ic),'ZData',zeros(size(zd)));
                end
            end
        end
    else
        % 老版本 contourgroup：直接改子对象 ZData
        ch = get(h,'Children');
        for ic = 1:numel(ch)
            zd = get(ch(ic),'ZData');
            set(ch(ic),'ZData',zeros(size(zd)));
...
for i=1:4
    Zt = amp(i) * gauss2(xg,yg,tops{i}(1),tops{i}(2),sigT(i));
    s1 = surf(xg,yg,Zt, 'EdgeColor','none', 'FaceAlpha',0.35);
end
shading interp; camlight headlight; lighting gouraud;

% ---------- 5) 轴、视角与观感 ----------
xlim([-3 3]); ylim([-3 3]); zlim([0 1.25]); view(-67,22);
axis vis3d; daspect([1 1 0.8]);
set(gca,'FontName','Arial','FontSize',11,'LineWidth',0.8);
xlabel('x'); ylabel('y'); zlabel('score / \psi');

% 网格和背景风格
grid on; set(gca,'GridAlpha',0.2,'MinorGridAlpha',0.15);
