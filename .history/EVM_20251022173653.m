...
% 预分配等高线句柄数组，便于后续置顶
hContours = gobjects(size(EV,1),1);
...
% 标出 A 与问号
text(-0.5,-0.5,0,'A','FontWeight','bold','FontSize',20,'Color','k');
...
% 超高斯（控制穹顶侧壁陡峭度，p>2 越陡）
superg = @(X,Y,cx,cy,s,p) exp( - ( sqrt((X-cx).^2 + (Y-cy).^2) ./ s ).^p );
tops  = {C_star, C_diam, C_square, C_dot};
sigT  = [0.65 0.55 0.55 0.60];
amp   = [1.05 0.95 0.90 0.85];
powT  = [2.6  2.4  2.4  2.6];   % 每个穹顶的形状指数（可调）
...
for i=1:4
    ...
    Zt = amp(i) * superg(xgi,ygi,cx,cy,sigT(i),powT(i));
    % 用 Zt 作为颜色，Zt+zlift 作为高度，从而抬高且保留原色（顶部红，底部蓝）
    s1 = surf(xgi,ygi, Zt + zlift, Zt, 'EdgeColor','none', 'FaceAlpha',0.35);
end
shading interp; camlight headlight; lighting gouraud;

% 先将等高线置于表面之上，再将散点置于最上层
uistack(hContours, 'top');
