...
def compute_feature_for_segment_arrays(Z: np.ndarray, x_1d: np.ndarray) -> np.ndarray:
    """
    Build the same-dimension feature vector as compute_feature_for_name, but from a
    provided STFT magnitude matrix Z and a 1D time signal x_1d (already matched to Z).

    Z: |STFT| matrix of shape (F, T)
    x_1d: time-domain 1D signal aligned with Z's time axis (only used for base stats)
    """
    Z = np.asarray(Z)
    x = np.squeeze(np.asarray(x_1d))
    ...
    # Time-domain statistics for 1D signal as a proxy for multi-channel medians
    try:
        kurt_time = float(sp_kurtosis(x, fisher=True, bias=False)) if x.size > 3 else 0.0
    except Exception:
        kurt_time = 0.0
    try:
        skewness_time = float(sp_skew(x, bias=False)) if x.size > 2 else 0.0
    except Exception:
        skewness_time = 0.0

    # NMF decomposition on |Z|
    F, T = Z.shape
    nmf_n = max(1, min(int(nmf_n), min(F, T)))
    nmf_model = NMF(
        n_components=nmf_n,
    ...
    # Renyi entropy computed on the full normalized |Z|
    alpha = 2.0
    sZ = float(np.sum(Z))
    if sZ > 0.0:
        pZ = Z / sZ
    ...
    nmf_feat = np.concatenate(comp_feats, axis=0) if len(comp_feats) > 0 else np.zeros((0,), dtype=np.float32)
    full_feat = np.concatenate([base_feat.astype(np.float32, copy=False), nmf_feat], axis=0)
    return full_feat

def compute_feature_for_segment(name: str, start_sample: int, end_sample: int) -> np.ndarray:
    """
    Compute the same feature as compute_feature_for_name but restricted to a
    time segment [start_sample, end_sample) (samples, 0-based) for dataset 'name'.
    """
    fs, f, t, z, x, y = compute_stft(name)
    Z_full = np.abs(z)
    x_full = np.squeeze(x)

    # Derive time mask for STFT frames belonging to the requested sample range
    # t is in seconds; convert sample indices to time
    t0 = float(max(0, start_sample)) / float(fs)
    t1 = float(max(start_sample + 1, end_sample)) / float(fs)
    frame_mask = (t >= t0) & (t < t1)
    if not np.any(frame_mask):
        # Fallback: if mask is empty, take the nearest frame
        idx = np.argmin(np.abs(t - (t0 + t1) * 0.5))
        frame_mask = np.zeros_like(t, dtype=bool)
        frame_mask[idx] = True

    Z = Z_full[:, frame_mask]
    x_seg = x_full[max(0, start_sample):max(0, end_sample)]
    if x_seg.size == 0:
        x_seg = x_full[: max(1, int(0.1 * x_full.size))]

    # Base features
    base_feat = _fixed_length_feature_from_stft(Z, x_seg)

    # Time-domain stats on multi-channel X limited to the segment
    trainFile = ['../Data/matdata/' + name + '.mat']
    X, _ = read_mat(trainFile)
    s0 = max(0, start_sample)
    s1 = min(X.shape[0], max(s0 + 1, end_sample))
    X_seg = X[s0:s1, :]
    ...
    for col in X_seg.T:
        ...
        kurt_list.append(k)
        skew_list.append(s)
    kurt_time = float(np.nanmedian(np.array(kurt_list))) if len(kurt_list) > 0 else 0.0
    skewness_time = float(np.nanmedian(np.array(skew_list))) if len(skew_list) > 0 else 0.0
