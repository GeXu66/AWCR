...
def _remove_unknown_models(models_dir: str) -> None:
    targets = ['Unknown1.mat', 'Unknown2.mat', 'Unknown3.mat']
    for fn in targets:
...
def pipeline(trainlist: List[List[str]],
    ...
    _remove_unknown_models(models_dir)
    ...
    for test_names in testlist:
        ...
        if cls_hat == 'Unknown':
            ...
            # Decide which model to use for estimation: prefer existing unknown model, otherwise fallback class model
            Theta_use = None
            if manager.exists(new_unknown):
            ...
            # Update EVM with this new unknown sample's features if supported
            try:
            ...
            out_dir = os.path.join(results_dir, safe_test_dir)
...
if __name__ == '__main__':
    ...
    pipeline(trainlist, testlist, channel_in, channel_out, past_order, future_order, mean_flag, results_dir=results_dir)
